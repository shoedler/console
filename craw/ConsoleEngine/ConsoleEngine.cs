...
namespace Craw.ConsoleEngine
{
    ...
    public struct CharItem
    {
        ...
        {
            get => (EConsolePixelColor)Attributes;
    ...
    class ConsoleFrame
    {
        ...
        /// <summary>
        /// Constructs a new <code>ConsoleFrame</code> from a string array.
        /// </summary>
        /// <param name="lines"></param>
        /// <returns></returns>
        public static ConsoleFrame FromStringArray(string[] lines)
        {
            var longestLine = lines.Max(l => l.Length);
            var frame = new ConsoleFrame((short)longestLine, (short)lines.Length);
            for (var i = 0; i < lines.Length; i++)
            {
                frame.InsertString(0, i, lines[i]);
            }
            return frame;
        }
        ...
        /// <summary>
        /// Inserts a <code>ConsoleFrame</code> at position <code>x</code>, <code>y</code>.
        /// </summary>
        /// <param name="x"></param>
        /// <param name="y"></param>    
        /// <param name="frame"></param>
        /// <exception cref="OverflowException">Thrown if the Frame exceeds the current Console Window size.</exception>
        public void InsertFrame(int x, int y, ConsoleFrame frame)
        {
            if (x + frame.Width > Width) throw new OverflowException($"Frame Width {frame.Width} at X {x} exceeds Target Frame Width {Width}");
            if (y + frame.Height > Height) throw new OverflowException($"Frame Height {frame.Height} at Y {y} exceeds Target Frame Height {Height}");

            for (var i = 0; i < frame.Width; i++)
            {
                for (var j = 0; j < frame.Height; j++)
                {
                    this[x + i, y + j] = frame[i, j];
                }
            }
        ...
        public ref CharItem this[int x, int y] => ref Buf[x + Width * y];
        ...
        /// </summary>
        /// <value>Returns a reference to a CharItem</value>
        public ref CharItem this[Coord coord] => ref Buf[coord.X + Width * coord.Y];
